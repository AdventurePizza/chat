...
export const Login = ({ beginTour, hideModal }: ILoginProps) => {
	...
	return (
		<div className="login-modal-container">
			...
			{showMetamask ? (
				<MetamaskTutorial setShowMetamask={setShowMetamask} />
			) : null}
		</div>
	);
};
...
const MetamaskTutorial = ({ setShowMetamask }: IMetamaskTutorialProps) => {
	...
	return (
		<div className="login-modal-container">
			...
			<div className="metamask-steps-container">
				{page === 1 ? (
					...
					<>
						...
						<p className="metamask-step-5">
							Step 5. Open metamask.io on chrome.
						</p>
						<p>Step 6. Set up password, store key phrase somewhere secure.</p>
						<p className="metamask-step-7">
							Step 7. Switch to matic mainnet or add matic.
						</p>
						<p className="metamask-step-details">
							Network name: polygon, customer rpc:
							https://rpc-mainnet.matic.network, chain id: 137, symbol: MATIC,
			...
			{page === 1 ? (
				<button className="login-button-email" onClick={() => setPage(2)}>
					Next
				</button>
			) : (
				<button
